...
public class DrainCommand {
    ...
    public static int drain(CommandSourceStack src) throws CommandSyntaxException {
        ...
        if (slot == -1) {
            src.sendSystemMessage(Component.literal("You don't have any space in your inventory!").withColor(TextColor.RED));
            return 1;
        }

        int score = infuse.dataManager().getScore(player);
        if (score <= 0) {
            src.sendSystemMessage(Component.literal("You don't have any positive effects to drain!").withColor(TextColor.RED));
            return 1;
        }
